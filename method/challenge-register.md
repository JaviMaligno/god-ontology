...
| CH21 | What criterion secures the identity of a post-mortem subject with its embodied predecessor under each open realization model, and can gappy existence of one individual be coherent? | personal continuation, mind–body realization, individuation, divine re-realization, P-gap closure | `tested` (partial) |
| CH22 | How do reductions of diachronic or evaluative authorship by created curators — propaganda, engineered addiction, algorithmic environments, coercive institutions — bear on responsibility by degree, and where is the line between formation and manipulation? | agency ecology, manipulation, responsibility, finite norm construction, moral status | `scoped` |
...
S44–S46 open a question-driven route into the register: plain questions from the accessible track that the ledger cannot answer are recorded as challenges. S45 gives the contingency/ground junction its F2 comparison; S46 earns a good-oriented divine profile toward sourcehood and relation and isolates the P-gap as CH18; S47 tests CH18 partially through permission, policy-level covers, and the bearer-level remainder, and opens CH20 because closure depends on personal continuation. S48 tests CH20 partially: continuation is coherent under every open realization model, within divine power as a further act, empirically undefeated and unsupported, and conditional on $GoodOrient_P(G)$; its identity burden becomes CH21, which waits on the mind–body branch. The track returns to CH12, where S30–S33 already supply the F2 constitution, boundary, and power results; S49 deepens the constitution/production junction. S50 then structures C118: cumulative context control can remove diachronic and evaluative authorship while token autonomy holds, which extends the agency-preservation presumption to the diachronic level and opens CH22 for created-level curation. S51 tests CH21 partially by deriving simple persistence from the agency model. CH19 records the experiential question.
...
S55 supplies the modal-status component: the possibility of imagined content is fixed by $Poss_C^{ind}$ (C491), imaginability is defeasible finite evidence of possibility rather than its constitution (C493), and the act's actuality never confers possibility on its content (C497). The ontology of content, reference to fictions and impossibilities, and the causal efficacy of thought remain open.
...
S50, developed in parallel on the question-driven track, is reconciled with this result in S58 (C504): the $AE$ profile and the three standings are canonical; S50's funnelling (profile-level closure of $OR$ over a history, which need not be adaptive), evaluative authorship, the rational/a-rational distinction, and the diachronic component of the source cover are retained as refinements, and its created-level question is CH22.
...
S46 establishes $GoodOrient_S(G)\land GoodOrient_R(G)$ and finds no established divine disposition toward the valence of created subjects. The challenge is not the classical inference from evil to non-existence, which does not run against a $G$ whose full goodness and unrestricted power are unearned; it is whether the good-oriented profile can be extended to $P$ at all.
...
S47 supplies that partial result. Cases 1 and 3 receive the policy-level source cover and case 2 the regularity cover; both leave a threshold residue whose arbitrary-threshold reply is recorded as contested. Case 4 is defined as gratuitous suffering. The bearer-level remainder shows that policy-level covers cannot by themselves close the gap for a bearer whose life ends in uncovered suffering, which opens CH20. The challenge remains `tested` rather than `resolved`.

## CH19 — Veridicality of experiential access

S44 distinguishes experience veridical of standing presence, veridical of exercised regard, and non-veridical. The challenge is to specify, beyond that partition, which features of an experience or of the subject's practice bear on veridicality and which defeat it, without making the experiential register either self-authenticating or empty.

Tests should include experiences with known naturalistic triggers, experiences that conflict with established truths, experiences across traditions with incompatible contents, cumulative practice versus isolated episodes, and the difference between an experience of presence and a claimed propositional revelation. The framework must keep C431: no experience is evidence for the architecture.

## CH20 — Personal continuation and bearer horizons

S47 derives that closure of the P-gap for bearers whose lives end in uncovered suffering requires undefeated $P$-fit within a horizon extending beyond the bearer's life. The challenge is whether the ontology can support such a horizon without importing it as a rescue.
...
S48 supplies the partial result: continuation is defined as the numerically same bearer with an undefeated fit horizon anywhere in $\mathfrak R^*$; every open realization model admits it by persistence or re-realization; it is typed as an intervention or further creative act within qualified power iff its identity condition is coherent; empirical base-dependence does not defeat it and nothing supports it; and its only internal support is the conditional from $GoodOrient_P(G)$. Universality and co-production follow if it obtains. The identity criterion is CH21.

## CH21 — Identity across the cessation of the realization base
...
Tests should include: two candidates equally continuous with $B$; a re-realization with partial psychological continuity; persistence of a bearer with no realized capacities; whether $G$'s individuating knowledge (C122) can fix identity or only track it; and whether the bearer-indexed $Fit_*$ can be satisfied by anything less than numerical identity. A negative result leaves C448 conditional on an incoherent consequent and reopens the P-gap.

S51 supplies the partial result. The agency model already entails simple persistence (C469), so no criterion is needed; gappy existence is coherent under E+ (C471); re-realization targets the individual de re while ACO tracks the identity fact (C472); a replica is a distinct unit (C473); and identity is what matters for a source (C474). The reductive realization is disfavored (C475). Open: the choice between persistence and re-realization routes, wholly unrealized existence, and fission at the created level.


## CH22 — Created curation, formation, and responsibility

S50 types manipulation as cumulative a-rational influence that removes evaluative authorship, and funnelling as closure of the profile space, both compatible with token autonomy. The created-level challenge is to say when formation under dependence and constraint (C166) becomes curation, and how reduced diachronic or evaluative authorship bears on responsibility by degree (C38, deferred).

Tests should include: upbringing versus indoctrination; persuasion versus engineered appetite; recommender systems that shape salience; institutions that close profile space while preserving formal choice; agents who later endorse standards installed in them; and agents who break a funnel. The challenge must keep the graded character of C462 and must not infer reduced welfare importance from reduced authorship (CH10).
