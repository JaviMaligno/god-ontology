...
| CH16 | Does the ontology support an extended bearer horizon — personal continuation — sufficient to cover bearer-level suffering, and what would it require of E+, ACO, the embodied subject, and the privative model? | P-gap closure, personal identity, E+, ACO, embodied agency, hell as privation, divine goodness | `tested` (partial) |
| CH17 | What criterion secures the identity of a post-mortem subject with its embodied predecessor under each open realization model, and can gappy existence of one individual be coherent? | personal continuation, mind–body realization, individuation, divine re-realization, P-gap closure | `tested` (partial) |
| CH18 | How do reductions of diachronic or evaluative authorship by created curators — propaganda, engineered addiction, algorithmic environments, coercive institutions — bear on responsibility by degree, and where is the line between formation and manipulation? | agency ecology, manipulation, responsibility, finite norm construction, moral status | `scoped` |
...
S36 establishes $GoodOrient_S(G)\land GoodOrient_R(G)$ and finds no established divine disposition toward the valence of created subjects. The challenge is not the classical inference from evil to non-existence, which does not run against a $G$ whose full goodness and unrestricted power are unearned; it is whether the good-oriented profile can be extended to $P$ at all.
...
S37 supplies that partial result. Cases 1 and 3 receive the policy-level source cover and case 2 the regularity cover; both leave a threshold residue whose arbitrary-threshold reply is recorded as contested. Case 4 is defined as gratuitous suffering. The bearer-level remainder shows that policy-level covers cannot by themselves close the gap for a bearer whose life ends in uncovered suffering, which opens CH16. The challenge remains `tested` rather than `resolved`.

## CH15 — Veridicality of experiential access

S34 distinguishes experience veridical of standing presence, veridical of exercised regard, and non-veridical. The challenge is to specify, beyond that partition, which features of an experience or of the subject's practice bear on veridicality and which defeat it, without making the experiential register either self-authenticating or empty.

Tests should include experiences with known naturalistic triggers, experiences that conflict with established truths, experiences across traditions with incompatible contents, cumulative practice versus isolated episodes, and the difference between an experience of presence and a claimed propositional revelation. The framework must keep C292: no experience is evidence for the architecture.

## CH16 — Personal continuation and bearer horizons

S37 derives that closure of the P-gap for bearers whose lives end in uncovered suffering requires undefeated $P$-fit within a horizon extending beyond the bearer's life. The challenge is whether the ontology can support such a horizon without importing it as a rescue.
...
S38 supplies the partial result: continuation is defined as the numerically same bearer with an undefeated fit horizon anywhere in $\mathfrak R^*$; every open realization model admits it by persistence or re-realization; it is typed as an intervention or further creative act within qualified power iff its identity condition is coherent; empirical base-dependence does not defeat it and nothing supports it; and its only internal support is the conditional from $GoodOrient_P(G)$. Universality and co-production follow if it obtains. The identity criterion is CH17.

## CH17 — Identity across the cessation of the realization base
...
Tests should include: two candidates equally continuous with $B$; a re-realization with partial psychological continuity; persistence of a bearer with no realized capacities; whether $G$'s individuating knowledge (C122) can fix identity or only track it; and whether the bearer-indexed $Fit_*$ can be satisfied by anything less than numerical identity. A negative result leaves C309 conditional on an incoherent consequent and reopens the P-gap.

S41 supplies the partial result. The agency model already entails simple persistence (C330), so no criterion is needed; gappy existence is coherent under E+ (C332); re-realization targets the individual de re while ACO tracks the identity fact (C333); a replica is a distinct unit (C334); and identity is what matters for a source (C335). The reductive realization is disfavored (C336). Open: the choice between persistence and re-realization routes, wholly unrealized existence, and fission at the created level.


## CH18 — Created curation, formation, and responsibility

S40 types manipulation as cumulative a-rational influence that removes evaluative authorship, and funnelling as closure of the profile space, both compatible with token autonomy. The created-level challenge is to say when formation under dependence and constraint (C166) becomes curation, and how reduced diachronic or evaluative authorship bears on responsibility by degree (C38, deferred).

Tests should include: upbringing versus indoctrination; persuasion versus engineered appetite; recommender systems that shape salience; institutions that close profile space while preserving formal choice; agents who later endorse standards installed in them; and agents who break a funnel. The challenge must keep the graded character of C323 and must not infer reduced welfare importance from reduced authorship (CH10).
