...
| CH14 | What kind of reality belongs to imagination and thought: the actual mental act, its intentional content, and any merely represented or possible object? | consciousness, intentionality, modality, actual/non-actual distinction, causal efficacy | `tested F2; content and reference closed in S80; intentionality bridge F3` |
...
S75, on the question-driven branch, reached the same conditional closure in parallel by a weaker-premise route: a fact-level limiter condition ($Limiter_G$, C604) that every $NCCBlock$ must meet, and the no-limiter lemma from $MGC^+\land MUU$ (C605–C606). S76 reconciles the two: S73's $UCD_G$ and $NCCBlock$ are canonical; the branch's bridge and lemma are retained as refinements with their condition on $MUU$ recorded (C610).
...
S57 supplies the modal-status component: the possibility of imagined content is fixed by $Poss_C^{ind}$ (C528), imaginability is defeasible finite evidence of possibility rather than its constitution (C530), and the act's actuality never confers possibility on its content (C534). The ontology of content, reference to fictions and impossibilities, and the causal efficacy of thought remained open until S80.

S80 closes those parts at F2. An act has an actual act, an actual vehicle, an objective satisfaction condition (a specification, grounded in the divine nature by S79), and an actual referent if any (C626); reference is to actual items under an anchoring relation, and a non-actual-directed act is about a specification, a merely possible individual being a profile plus a label (C627); impossible contents are unsatisfiable conjunctions of satisfiable parts (C628); fictions are stipulated specifications with actual provenance (C629); divine knowledge of thought is ACO, MKO, and $ASA$ with no possibile to know (C630); thought is efficacious through its vehicle and content-explanation is constitutive (C631). What remains is the intentionality bridge, placed with phenomenality at F3.
