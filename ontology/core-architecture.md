...
S75, on the question-driven branch, analyzes the eighth row at the level of facts: a non-created concrete limits $G$ only if some contingent fact concerning it has no ultimate ground in $G$ ($Limiter_G$, C604), and every $NCCBlock$ requires such a fact. Under extended modal grounding coverage $MGC^+$ (C605) and $MUU$ read over the grounds of contingent facts, no limiter is possible ($NLL$, C606). S76 reconciles this with S73 below: $UCD_G$ is canonical and closes the existence question; the branch's lemma closes the blocker question one step earlier, at the cost of the stronger reading of $MUU$ (C610).
