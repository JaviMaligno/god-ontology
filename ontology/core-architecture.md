...
S78 tests whether change makes this privation necessary. Temporal embodied life of the selected kind is constitutively dynamic: metabolism, perception, action, learning, and development involve change. The unrestricted thesis that all time requires change remains open because relationist accounts and static-time countermodels disagree. More importantly, change alone does not entail death or loss of a due good; growth and rearrangement are counterexamples.
...
S78 distinguishes the bearer's onset from the onset of any person-phase or conscious capacity. A created subject begins with the first stage of its actual token career. Before that stage, modal space may contain an admissible type-profile but contains no pre-existing de re individual waiting to be selected. Creation grounds the bearer; numerical identity is the constitutive fact that the bearer is itself rather than another product added to it. Individuation is prior to the subject's acts in grounding order—the acts presuppose their bearer—not temporally prior to the bearer's first stage. The exact biological first stage remains open because it depends on when one unified organismic bearer satisfies the relevant individuation profile.

Eternalism does not make the bearer everlasting. The complete career and the identity of its stages are tenselessly actual in $H^*$, while $ExistsAt(B,t)$ can be false before its first stage, during a permitted gap, and after a final stage. Atemporal divine knowledge of that career does not transfer its mode to $B$. Thus the identity fact is tenseless, the bearer remains temporally located, and a post-gap stage counts as $B$ only under the one-career, origin, essence, and non-branching constraints below.
